...
class fileDAO:
    ...
    def insertPPTSUCaiFile(self, group, fileName, downLoadURL):
        self._cursor.execute(self.sql, [group, fileName, downLoadURL, 3]);
        self._conn.commit()

    ## 字体
    def insertPPTFontFile(self, group, fileName, downLoadURL):
        self._cursor.execute(self.sql, [group, fileName, downLoadURL, 4]);
        self._conn.commit()
        ## 字体
    ## 图表
    def insertPPTTableFile(self, group, fileName, downLoadURL):
        self._cursor.execute(self.sql, [group, fileName, downLoadURL, 5]);
        self._conn.commit()
